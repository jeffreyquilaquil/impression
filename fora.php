...
if ($GLOBALS['ad_ug_is_admin'] == 0){
    $where .= " AND (region_id = $GLOBALS[ad_u_region_id])";
}
...
$pg_title = "Fora/Trainings/Seminars";
...
<div class="panel panel-default">
    <div class="panel-heading clearfix">
        ...
        <div class="pull-right">
            ...
            ?>
            <a class="btn btn-primary btn-sm" href="fora_form.php?op=0&amp;id=0" title="Add Record"><span class="fa fa-plus"></span> Add Record</a>
            <?php
            }
            ?>
        </div>
    ...
    <div class="panel-body">
        <form method="POST" action="fora.php" accept-charset="UTF-8" class="form-inline" role="form">
            <div class="form-group">
                ...
                <div class="input-group input-group-sm">
                    <input class="form-control input-sm" placeholder="Forum Title ..." type="text" maxlength="255" name="q" id="q" value="<?php echo $q; ?>">
    ...
    <div class="alert alert-warning">Please try changing the filters and click on search if you can't find newly added records.</div>
        ...
        <table id="grid_table" class="table table-bordered table-striped table-hover table-condensed tablesorter">
        	...
        	if ($rows) {
                while($row = mysqli_fetch_array($rows)) {
        			...
        			$action	= '';
                    if (can_access('Fora', 'view')){
                        $action .= '<a class="btn btn-primary btn-xs" href="'.WEBSITE_URL.'fora_view.php?pid='.$row['fr_id'].'" title="Details"><span class="fa fa-folder-open"></span></a>  ';
                    }

                    if (can_access('Fora Documents', 'view')){
                        $action .= '<a class="btn btn-primary btn-xs" href="'.WEBSITE_URL.'fora_documents.php?pid='.$row['fr_id'].'" title="Documents"><span class="fa fa-file"></span></a>  ';
                    }

                    if (can_access('Fora', 'edit')){
                        $action .= '<a class="btn btn-primary btn-xs" href="'.WEBSITE_URL.'fora_form.php?op=1&amp;id='.$row['fr_id'].'" title="Edit"><span class="fa fa-pencil"></span></a>  ';
                    }

                    if (can_access('Fora', 'delete')){
                        $action .= '<a class="btn btn-primary btn-xs" href="javascript:void(0);" onclick="confirmDialog(\'Delete this record?\', \'Confirm Delete\', \''.WEBSITE_URL.'fora_delete.php?op=2&amp;id='.$row['fr_id'].'\');" title="Delete"><span class="fa fa-close"></span></a>';
                    }
