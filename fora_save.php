...
if (postEmpty('fr_requesting_party')){
	...
	redirect(WEBSITE_URL.'fora_form.php?op='.$op.'&id='.$id);
	die();
}

/*
if (postEmpty('col_id')){
	$_SESSION['errmsg'] = "Cooperating Agencies is required.";
	redirect(WEBSITE_URL.'fora_form.php?op='.$op.'&id='.$id);
	die();
}
*/
...
if (postEmpty('fr_start')){
	...
	$_SESSION['errmsg'] = "Start must be a datetime (mm/dd/yyyy hr:min am/pm).";
	redirect(WEBSITE_URL.'fora_form.php?op='.$op.'&id='.$id);
	die();
}
...
if (postEmpty('fr_location')){
	$_SESSION['errmsg'] = "Venue is required.";
	redirect(WEBSITE_URL.'fora_form.php?op='.$op.'&id='.$id);
	die();
}
