...
if (!can_access('Organization Categories', 'view')){
    redirect(WEBSITE_URL.'index.php');
}

$q_key = 'q_lu_organization_types';
$qg_key = 'qg_lu_organization_types';
...
$sql = "SELECT * FROM psi_organization_types";
$where = '';

if (strlen($q) > 0){
    $where .= "(ot_name like '%$q%')";
}
...
<div class="panel panel-default">
    <div class="panel-heading clearfix">
        ...
        <div class="pull-right">
            ...
            ?>
            <a class="btn btn-primary btn-sm" href="lu_organization_types_form.php?op=0&amp;id=0" title="Add Category"><span class="fa fa-plus"></span> Add Category</a>
            <?php
            }
            ?>
        </div>
    ...
    <div class="panel-body">
        <form method="POST" action="lu_organization_types.php" accept-charset="UTF-8" class="form-inline" role="form">
        	...
        	if ($rows) {
                while($row = mysqli_fetch_array($rows)) {
                    ...
                    if (can_access('Organization Categories', 'edit')){
                        $action .= '<a class="btn btn-primary btn-xs" href="'.WEBSITE_URL.'lu_organization_types_form.php?op=1&amp;id='.$row['ot_id'].'" title="Edit"><span class="fa fa-pencil"></span></a>  ';
                    }
                    if (can_access('Organization Categories', 'delete')){
                        $action .= '<a class="btn btn-primary btn-xs" href="javascript:void(0);" onclick="confirmDialog(\'Delete this record?\', \'Confirm Delete\', \''.WEBSITE_URL.'lu_organization_types_delete.php?op=2&amp;id='.$row['ot_id'].'\');" title="Delete"><span class="fa fa-close"></span></a>  ';
                    }
?>                <tr>
