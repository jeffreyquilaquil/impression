...
<div class="panel panel-default">
    <div class="panel-heading clearfix">
        ...
        <div class="pull-right">
            ...
            ?>
            <a class="btn btn-primary btn-sm" href="consultancies_form.php?op=0&amp;id=0" title=" Add Consultancy"><span class="fa fa-plus"></span> Add Consultancy</a>
            <?php
            }
    ...
    <div class="panel-body">
        <form method="POST" action="consultancies.php" accept-charset="UTF-8" class="form-inline" role="form">
        ...
        <table id="grid_table" class="table table-bordered table-striped table-hover table-condensed tablesorter">
        	...
        	if ($rows) {
                while($row = mysqli_fetch_array($rows)) {
                    ...
                    if (can_access('Consultancies', 'view')){
                        $action .= '<a class="btn btn-primary btn-xs" href="'.WEBSITE_URL.'consultancies_view.php?pid='.$row['con_id'].'" title="View Details"><span class="fa fa-folder-open"></span></a>  ';
                    }

                    if (can_access('Consultancies', 'edit')){
                        $action .= '<a class="btn btn-primary btn-xs" href="'.WEBSITE_URL.'consultancies_form.php?op=1&amp;id='.$row['con_id'].'" title="Edit"><span class="fa fa-pencil"></span></a>  ';
                    }

                    if (can_access('Consultancies', 'delete')){
                        $action .= '<a class="btn btn-primary btn-xs" href="javascript:void(0);" onclick="confirmDialog(\'Delete this record?\', \'Confirm Delete\', \''.WEBSITE_URL.'consultancies_delete.php?op=2&amp;id='.$row['con_id'].'\');" title="Delete"><span class="fa fa-close"></span></a>';
                    }
...
?>                <tr>
                    ...
                    <td class="nowrap"><?php echo $row['coop_name']; ?></td>
...
<?php
    ...
    function getImpYearOptions($value){
        $s = '';
        $sql = "SELECT DISTINCT con_end_yr FROM vwpsi_consultancies where prj_id = 0 ORDER BY con_end_yr DESC";
