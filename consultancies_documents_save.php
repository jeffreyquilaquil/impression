...
if ($op == 0){
	if ($_FILES[$file_index]['error'] == UPLOAD_ERR_NO_FILE){
		...
		redirect(WEBSITE_URL.'consultancies_documents_form.php?pid='.$pid.'&op='.$op.'&id='.$id);
		die();
	}
}
...
if ($_FILES[$file_index]['error'] != UPLOAD_ERR_NO_FILE){
	if (($_FILES[$file_index]['error'] == UPLOAD_ERR_INI_SIZE) || ($_FILES[$file_index]['error'] == UPLOAD_ERR_FORM_SIZE)){
		...
		redirect(WEBSITE_URL.'consultancies_documents_form.php?pid='.$pid.'&op='.$op.'&id='.$id);
		die();
	} elseif (	(stripos($_FILES[$file_index]['type'], "msword") === false) && 
				(stripos($_FILES[$file_index]['type'], "ms-word") === false) &&
				(stripos($_FILES[$file_index]['type'], "ms-excel") === false) &&
				(stripos($_FILES[$file_index]['type'], "ms-powerpoint") === false) &&
				(stripos($_FILES[$file_index]['type'], "wordprocessing") === false) &&
				(stripos($_FILES[$file_index]['type'], "spreadsheet") === false) &&
				(stripos($_FILES[$file_index]['type'], "pdf") === false) &&
				(stripos($_FILES[$file_index]['type'], "presentation") === false) &&
				(stripos($_FILES[$file_index]['type'], "jpg") === false) &&
				(stripos($_FILES[$file_index]['type'], "jpeg") === false) &&
				(stripos($_FILES[$file_index]['type'], "png") === false) &&
				(stripos($_FILES[$file_index]['type'], "gif") === false) &&
				(stripos($_FILES[$file_index]['type'], "bmp") === false) 
			 ){
