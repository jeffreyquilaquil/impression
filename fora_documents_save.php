...
saveFormCache('psi_fora_documents');
...
$file_index = 'frdoc_file';

if ($op == 0){
	if ($_FILES[$file_index]['error'] == UPLOAD_ERR_NO_FILE){
		$_SESSION['errmsg'] = "Document is required.";
		redirect(WEBSITE_URL.'fora_documents_form.php?pid='.$pid.'&op='.$op.'&id='.$id);
		die();
	}
}
...
if ($_FILES[$file_index]['error'] != UPLOAD_ERR_NO_FILE)
{
	if (($_FILES[$file_index]['error'] == UPLOAD_ERR_INI_SIZE) || ($_FILES[$file_index]['error'] == UPLOAD_ERR_FORM_SIZE)){
		...
		$_SESSION['errmsg'] = "Invalid file type - " . strpos($_FILES[$file_index]['type'], "image/jpeg");
		redirect(WEBSITE_URL.'fora_documents_form.php?pid='.$pid.'&op='.$op.'&id='.$id);
		die();
	}	
	...
	$GLOBALS['frdoc_filename'] = $filename;
	// echo $dest_path.'<br>';
	...
	unset($GLOBALS[frdoc_filename]);
	unset($GLOBALS[frdoc_file]);
}
...
if ($op == 1){
	$sql = getUpdateQuery('psi_fora_documents', 'frdoc_id');
	...
	$sql = getInsertQuery('psi_fora_documents', 'frdoc_id');
	mysqli_query($GLOBALS['cn'], $sql);
	$msg = 'Record Added.';
}
